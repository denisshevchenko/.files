...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    --
    -- Change keyboard layout, left Ctrl+Shift
    [((controlMask, xK_Shift_L), spawn "/home/denis/.local/bin/changeKbdLayout.sh")]
    ++
...
mediaKeys = [ ("<XF86AudioLowerVolume>",  lowerVolume 10 >> return ())
            ...
            , ("<XF86MonBrightnessDown>", spawn "brightnessctl -q set 10%-")
            , ("<XF86MonBrightnessUp>",   spawn "brightnessctl -q set 10%+")
            -- Take a screenshot
            , ("<Print>",                 spawn "maim ~/Pictures/screenshot_$(date +%F_%H%M%S).png")
            ]
...
--
main = do
  xmobarProc <- spawnPipe "xmobar ~/.xmonad/xmobarrc.hs"
  xmonad $ docks $ defaults { logHook = do
    ...
    wallpaperSetter defWallpaperConf -- The simplest soluiton: one fixed background picture for all workspaces.
      { wallpapers = defWPNames myWorkspaces
                       <> WallpaperList wallpapersImages
      }
  }
